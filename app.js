...
const FIREWORK_ACCELERATION = 8.05;
...
const TICKS_PER_FIREWORK_AUTOMATED_MIN = 5;
...
function Particle(x, y) {
    ...
    // Set gravity.
    this.gravity = random(-PARTICLE_GRAVITY,PARTICLE_GRAVITY);
...
Particle.prototype.update = function (index) {
    ...
    this.x += Math.cos(this.angle) * this.speed + this.gravity*random(-5,5);
    this.y += Math.sin(this.angle) * this.speed + this.gravity*random(-5,5);
...
function launchAutomatedFirework() {
    ...
    if (ticksSinceFireworkAutomated >= random(TICKS_PER_FIREWORK_AUTOMATED_MIN, TICKS_PER_FIREWORK_AUTOMATED_MAX)) {
        ...
        if (!isMouseDown) {
            ...
            let endY = random(0, canvas.height);
